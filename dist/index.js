...
function isQdrantStoreArguments(args) {
    return typeof args === 'object' && args !== null && 'information' in args;
}
function isQdrantFindArguments(args) {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            if (request.params.name === 'qdrant_store') {
                ...
                if (!isQdrantStoreArguments(args)) {
                    throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for qdrant_store: ${JSON.stringify(args)}`);
                }
                ...
                if (!isQdrantFindArguments(args)) {
                    throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for qdrant_find: ${JSON.stringify(args)}`);
                }
                const { query } = args;
